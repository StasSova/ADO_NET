...
public class DataBaseAPI_D : IDataBaseAPI
{
    ...
    public Task<ICollection<T>> Get<T>() where T : DbEntity
    {
        return Task.FromResult<ICollection<T>>([.. Context.Set<T>()]);
    }
    public Task<ICollection<T>> Get<T>(int page, int entitiesPerPage = 10) where T : DbEntity
    {
        if (page <= 0)
        ...
        return Task.FromResult<ICollection<T>>([.. Context.Set<T>().Skip((page - 1) * entitiesPerPage).Take(entitiesPerPage)]);
    }
    public Task<ICollection<TEntity>> Get<TEntity, TValue>(string propertyName, TValue propertyValue) where TEntity : DbEntity
    {
        ...
        return Task.FromResult<ICollection<TEntity>>([.. filteredEntities]);
    }
    public Task<ICollection<TEntity>> Get<TEntity>(Dictionary<string, (object value, bool isExactMatch)> propertyFilters) where TEntity : DbEntity
    {
        // Получаем набор сущностей типа TEntity из контекста базы данных
        DbSet<TEntity> entities = Context.Set<TEntity>();
        ...
        // Выполняем запрос к базе данных и получаем список отфильтрованных сущностей и
        // возвращаем список отфильтрованных сущностей
        return Task.FromResult<ICollection<TEntity>>(filteredEntities.ToList());
    }
}