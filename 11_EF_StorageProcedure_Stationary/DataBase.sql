...
WHERE dbid = db_id('04_StationeryCompany');
...
go
-- Отображение всех менеджеров по продажам
CREATE PROCEDURE ShowAllCompnayWithID
AS
BEGIN
    SELECT * FROM Company;
END;
...
CREATE PROCEDURE ShowAllHistoryWithID
AS
BEGIN
    SELECT * FROM HistoryOfSells;
END;
go
...
CREATE PROCEDURE ShowItemsInformationByType
	@TypeId INT
AS
BEGIN
    SELECT * FROM Item i
...
--drop procedure UpdateCompanyInformation
...
--drop procedure UpdateManagerInformation
...
--drop procedure UpdateItemTypeInformation
