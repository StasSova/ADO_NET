...
public class DataBaseAPI_D : IDataBaseAPI
{
    ...
    public async Task AddBookToUserCart(int userId, int bookId)
    {
        try
        {
            ...
            if (userCart != null)
            {
                var book = await Context.BooksForSale
                    .Where(b => b.Id == bookId)
    ...
    public async Task<ICollection<M_BookForSale>> GetBooksForSaleByGenre(int genreId)
    {
        try
        {
            return await Context.BooksForSale.Where(x => x.Ganres.Any(x => x.Id == genreId)).ToListAsync();
        }
        catch (Exception ex)
        {
    ...
    public async Task<ICollection<M_BookForSale>> GetPromotionBooks()
    {
        try
        {
            return await Context.BooksForSale.Where(x => x.Discount > 0).ToListAsync();
        }
        catch (Exception ex)
        {
            Console.WriteLine(ex.Message);
    ...
    public async Task<ICollection<M_BookForSale>> GetBooksForSaleByAuthor(int authorId)
    {
        try
        {
            return await Context.BooksForSale.Where(x => x.AuthorId == authorId).ToListAsync();
        }
        catch (Exception ex)
        {
            Console.WriteLine(ex.Message);
